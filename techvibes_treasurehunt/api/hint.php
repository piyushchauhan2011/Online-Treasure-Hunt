...
try {
    ...
    elseif ($current === null || (int) $current['id'] !== (int) $question['id']) {
        ...
        $progress = progressForQuestion($pdo, $userId, $questionId);
        if ((int) $progress['hints_used'] === 0) {
            $update = $pdo->prepare("UPDATE user_question_progress SET hints_used = 1, points_awarded = -50 WHERE user_id = :user_id AND question_id = :question_id AND status = 'in_progress'");
            $update->execute(['user_id' => $userId, 'question_id' => $questionId]);
            $result = [['ok' => true, 'hint' => $question['hint_text'], 'score' => totalScore($pdo, $userId), 'next_question_id' => (int) $question['id']], 200];
        } else {
            $update = $pdo->prepare("UPDATE user_question_progress SET status = 'skipped', points_awarded = 0, completed_at = NOW() WHERE user_id = :user_id AND question_id = :question_id AND status = 'in_progress'");
            $update->execute(['user_id' => $userId, 'question_id' => $questionId]);
            if ($update->rowCount() !== 1) { $result = [['ok' => false, 'error' => 'Question is already completed.'], 409]; }
            else {
                $next = currentQuestion($pdo, $userId);
                $result = [['ok' => true, 'hint' => null, 'score' => totalScore($pdo, $userId), 'next_question_id' => $next ? (int) $next['id'] : null], 200];
            }
        }
    }
    $pdo->commit();
